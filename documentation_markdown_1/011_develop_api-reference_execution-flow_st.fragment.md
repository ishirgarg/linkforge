...
This documentation covers the `st.fragment` function in Streamlit.
```
...
*   **run\_every** (`int`, `float`, `timedelta`, `str`, or `None`): The time interval between automatic fragment reruns. This can be one of the following:
    *   `None` (default).
    *   An `int` or `float` specifying the interval in seconds.
    *   A string specifying the time in a format supported by [Pandas' Timedelta constructor](https://pandas.pydata.org/docs/reference/api/pandas.Timedelta.html), e.g. "1d", "1.5 days", or "1h23s".
